(ns electrical-equipment-repair.store-contract-test
  ...
  contract-test`/`other-equipment-repair.store-contract-test` for the
  same pattern on the sibling actors."
...
(deftest read-parity
  (doseq [[label s] (backends)]
    (testing label
      (is (= "Kōji Electric Motor Works -- 3-Phase Induction Motor Rewind (work order #5521)" (:name (store/equipment s "equipment-1"))))
    ...
    (testing label
      (testing "log-repair-record commits a patch and appends a repair-record-log entry"
        (store/commit-record! s {:op :log-repair-record :path ["equipment-1"]
                                 :value {:id "equipment-1" :diagnostic-notes "loose terminal lug"}})
        (is (= "loose terminal lug" (:diagnostic-notes (store/equipment s "equipment-1"))))
        (is (= "Kōji Electric Motor Works -- 3-Phase Induction Motor Rewind (work order #5521)" (:name (store/equipment s "equipment-1"))) "unrelated field preserved")
      ...
      (testing "flag-safety-concern sets safety-concern-unresolved? true, drafts a record + notice document"
        (store/commit-record! s {:op :flag-safety-concern :path ["equipment-1"]
                                 :value {:equipment-id "equipment-1" :concern-description "test insulation crack"}})
      ...
      (testing "order-supplies drafts a record and advances the sequence"
        (store/commit-record! s {:op :order-supplies :path ["equipment-1"]
                                 :value {:equipment-id "equipment-1" :items ["terminal-lug"] :cost-usd 300}})
      ...
      (testing "sequences are jurisdiction-scoped, not global"
        (store/commit-record! s {:op :log-repair-record :path ["equipment-5"]
                                 :value {:id "equipment-5" :diagnostic-notes "control panel wiring misaligned"}})
