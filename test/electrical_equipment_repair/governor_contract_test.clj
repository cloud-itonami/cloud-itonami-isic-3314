(ns electrical-equipment-repair.governor-contract-test
  "The governor contract as executable tests -- the electrical-equipment-
  repair-coordination analog of `installation.governor-contract-test`/
  `other-equipment-repair.governor-contract-test`. The single invariant
  under test:
...
(deftest clean-log-repair-record-auto-commits
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-repair-record :subject "equipment-1" :patch {:id "equipment-1" :diagnostic-notes "loose terminal lug"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "loose terminal lug" (:diagnostic-notes (store/equipment db "equipment-1"))) "SSoT actually updated")
...
(deftest fabricated-jurisdiction-is-held
  (testing "equipment-2 (ATL, no spec-basis in electrical-equipment-repair.facts) -> HOLD, never reaches a human"
    (let [[db actor] (fresh)
  ...
  (testing "equipment-1 is fully clean -- :flag-safety-concern STILL always interrupts, unconditionally"
    (let [[db actor] (fresh)
          r1 (exec-op actor "t10" {:op :flag-safety-concern :subject "equipment-1"
                                   ...
                                   :concern-description "winding insulation resistance below spec, arc-flash risk"} operator)]
...
(deftest flag-safety-concern-triggers-notification-only-after-approval
  (let [[_db actor] (fresh)
        r1 (exec-op actor "t11" {:op :flag-safety-concern :subject "equipment-1"
                                 :concern-type :arc-flash-risk :concern-description "exposed bus bar, panel cover missing"} operator)]
...
(deftest order-supplies-below-threshold-auto-commits
  (let [[db actor] (fresh)
        res (exec-op actor "t12" {:op :order-supplies :subject "equipment-1"
                                  :items ["terminal-lug"] :cost-usd 300 :vendor "Local Electrical Supply Co."} operator)]
    (is (= :commit (get-in res [:state :disposition])))
...
(deftest order-supplies-above-threshold-escalates
  (let [[db actor] (fresh)
        r1 (exec-op actor "t13" {:op :order-supplies :subject "equipment-1"
                                 :items ["replacement-stator-core"] :cost-usd 9000 :vendor "Heavy Electric Rebuild Supply"} operator)]
...
(deftest forbidden-action-class-markers-are-permanent-hard-violations
  (doseq [marker [:repair-equipment-control? :diagnostic-tool-control? :direct-actuation?
                  :return-to-service-sign-off? :re-energization-sign-off?]]
    (testing marker
      (let [db (store/seed-db)
...
(deftest approver-rejection-is-held-not-committed
  (let [[db actor] (fresh)
        r1 (exec-op actor "t15" {:op :flag-safety-concern :subject "equipment-1"
                                 :concern-type :insulation-failure :concern-description "test"} operator)]
    (is (= :interrupted (:status r1)))
...
(deftest schedule-repair-operation-is-auto-eligible-at-phase-3-unlike-3320
  (testing "structural invariant: this actor's escalation surface auto-commits a governor-clean schedule at phase 3, the same shape cloud-itonami-isic-3319 uses"
