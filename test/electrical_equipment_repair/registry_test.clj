(ns electrical-equipment-repair.registry-test
  (:require [clojure.test :refer [deftest is]]
            [electrical-equipment-repair.registry :as r]))
...
(def sample-equipment
  {:id "equipment-1" :name "Kōji Electric Motor Works -- 3-Phase Induction Motor Rewind (work order #5521)" :jurisdiction "JPN"})

(deftest safety-concern-notice-cites-legal-basis-inline
  (let [doc (r/render-safety-concern-notice sample-equipment "JPN-SCF-000000" "巻線の絶縁抵抗値が基準未満")]
    (is (re-find #"JPN-SCF-000000" doc))
    (is (re-find #"労働安全衛生規則" doc))
    (is (re-find #"laws\.e-gov\.go\.jp" doc))
    (is (re-find #"絶縁抵抗値" doc))
    (is (re-find #"licensed repair" doc) "honestly disclaims return-to-service/re-energization sign-off authority")))
