...
`cloud-itonami-isic-3314` accepts contributions to the OSS blueprint,
...
- Never extend this actor's op-allowlist, advisor or governor to permit
  repair-equipment/diagnostic-tool control or return-to-service/
  re-energization sign-off authority. That is a hard scope boundary, not
  a rollout milestone -- proposals to relax it should be rejected, not
  merged.
...
- Extending `electrical-equipment-repair.facts` coverage to a new
...
- Do not add JVM-only interop to `src/` (no `java.net.http`, no other
  ...
  added, must go behind `electrical-equipment-repair.notify/Notifier`
