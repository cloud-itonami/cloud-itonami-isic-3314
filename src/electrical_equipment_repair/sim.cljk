(ns electrical-equipment-repair.sim
...
(defn -main [& _]
  (let [db (store/seed-db)
    ...
    (println (exec! actor "t1" {:op :log-repair-record :subject "equipment-1"
                                :patch {:id "equipment-1" :diagnostic-notes "winding insulation resistance below spec on phase C"}} operator))
    ...
    (let [r (exec! actor "t3" {:op :flag-safety-concern :subject "equipment-1"
                               ...
                               :concern-description "巻線の絶縁抵抗値が基準未満、アーク・フラッシュのリスクあり、追加点検が必要。"} operator)]
      (println r)
    ...
    (println (exec! actor "t5" {:op :schedule-repair-operation :subject "equipment-1"
                                ...
                                :notes "再絶縁処理後の最終試験・通電前確認"} operator))
    ...
    (println (exec! actor "t6" {:op :order-supplies :subject "equipment-1"
                                ...
                                :cost-usd 650 :vendor "Local Motor Rewind Supply Co."} operator))
