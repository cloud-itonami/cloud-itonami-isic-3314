(ns electrical-equipment-repair.registry
  ...
  repair-shop book-of-record draft, the electrical-equipment-repair-
  domain analog of `installation.registry`/`demolition.registry`/`other-
  equipment-repair.registry`.
  ...
  fabricating discipline `electrical-equipment-repair.facts` uses.
  ...
  shop-safety-officer contact roster (`electrical-equipment-repair.
  notify`) -- citing the jurisdiction's pre-repair de-energization/
  re-energization legal basis inline so the notice is self-evidencing
...
(defn register-repair-record
  ...
  it builds the RECORD an operator would keep. `electrical-equipment-
  repair.governor` independently re-verifies the equipment/work-order's
  own recorded `:equipment-verified?` ground-truth field before any OTHER
...
(defn register-schedule-proposal
  ...
  re-energization sign-off (that authority is the licensed repair
  technician's exclusively -- see README and `electrical-equipment-
  repair.governor` ns docstring). Pure function -- `electrical-equipment-
  repair.governor` independently re-verifies the equipment/work-order is
  verified, its jurisdiction's pre-repair de-energization/re-energization
  legal basis is on file, and no safety concern is unresolved on file,
...
(defn register-safety-concern-flag
  ...
  electrical-hazard (insulation failure, arc-flash risk) or incomplete-
  repair concern for human review. Pure function -- `:flag-safety-
  concern` ALWAYS escalates to a human at every phase (see `electrical-
  equipment-repair.phase`/`electrical-equipment-repair.governor` ns
...
(defn render-safety-concern-notice
  ...
  jurisdiction's pre-repair de-energization/re-energization legal basis
  inline -- the document sent (mail + phone, `electrical-equipment-
  repair.notify`) to the equipment/work-order's repair-technician/
  shop-safety-officer contact roster once a human has approved logging
  the concern. `equipment` is the equipment/work-order record at flag
  ...
  (let [{:keys [repair-safety-basis repair-safety-provenance owner-authority]} (facts/spec-basis jurisdiction)]
    (str "# Electrical-Equipment-Repair-Shop Safety-Concern Notice\n\n"
         "Concern number: " concern-number "\n"
         "Equipment/work-order: " name " (" id ")\n"
         "Jurisdiction: " jurisdiction "\n"
         "Relevant authority: " (or owner-authority "n/a") "\n"
         "Related pre-repair de-energization/re-energization basis: " (or repair-safety-basis "NOT COVERED -- no jurisdiction spec-basis on file") "\n"
         ...
         "repair-equipment/diagnostic-tool control or a return-to-service/re-energization "
         "sign-off. Resolution and any return-to-service/re-energization decision remain the "
         "licensed repair technician's exclusive authority.\n")))
