(ns electrical-equipment-repair.governor
  "Repair Governor -- the independent compliance layer named in
  `blueprint.edn` (`:itonami.blueprint/governor :electrical-equipment-
  repair-governor`) that earns the Repair Advisor the right to commit.
  The LLM has no notion of electrical-repair-shop safety law, whether an
  ...
  charter into repair-equipment/diagnostic-tool control or a
  return-to-service/re-energization sign-off, so this MUST be a separate
  system able to *reject* a proposal and fall back to HOLD -- the
  electrical-equipment-repair analog of `cloud-itonami-isic-3320`'s
  Installation Governor and `cloud-itonami-isic-3319`'s Repair Governor.

  ## Scope -- this is a COORDINATION-ONLY actor

  This actor NEVER controls repair equipment/diagnostic tools and NEVER
  signs off on return-to-service or re-energization -- that authority is
  the licensed repair technician's EXCLUSIVELY. Every proposal this
  ...
  artifact is now logged/scheduled/flagged/ordered', never 'a repair tool
  was operated', 'equipment is authorized to return to service' or
  'equipment has been re-energized'. Checks 1-4 below encode this scope
  as STRUCTURAL, permanent HARD holds -- not policy that could be relaxed
  by a future phase, unlike checks 5-6, which are ordinary
  per-jurisdiction/ground-truth safety gates.
  ...
  CANNOT override them.
    ...
    3. Forbidden action class    -- a proposal whose `:value` carries a
                                     ...
                                     service-sign-off?` / `:re-
                                     energization-sign-off?` marker true
                                     is rejected, unconditionally -- even
                                     though this actor's own mock advisor
                                     never sets these, the governor checks
                                     independently so a compromised
                                     advisor gains nothing by trying. The
                                     `:re-energization-sign-off?` marker
                                     is this actor's electrical-domain-
                                     specific addition to the forbidden
                                     set every `cloud-itonami-isic-*`
                                     repair actor shares -- electrical
                                     equipment repair distinguishes
                                     de-energization (before work) from
                                     re-energization (after work, the
                                     electrical analog of return-to-
                                     service; see `electrical-equipment-
                                     repair.facts` JPN/DEU entries) and
                                     both are the licensed repair
    ...
    6. Unresolved safety concern -- for `:schedule-repair-operation`, the
                                     ...
                                     `high-stakes` below and `electrical-
  ...
  jurisdictions), this actor's `electrical-equipment-repair.facts`
  catalog found NO `:quantitative` jurisdiction for the pre-repair
  de-energization/re-energization duty (see that ns docstring) -- so
  there is no numeric bright line to independently re-check here, and no
  seventh HARD check exists for it. This is an honest absence, not an
  oversight.
  ...
  the human may approve.
    ...
    - `:flag-safety-concern` is UNCONDITIONALLY a member of `high-stakes`
      ...
      confidence or governor cleanliness. Surfacing an electrical-hazard
      (insulation failure, arc-flash risk) or incomplete-repair concern
      is exactly the kind of judgment this actor must never let
      auto-commit.
    - `:schedule-repair-operation` is DELIBERATELY NOT a permanent member
      ...
      scheduling a diagnostic/repair/testing window for electrical
      equipment (motors, generators, transformers, switchgear) MAY
      auto-commit at phase 3 when the governor is completely clean (see
      `electrical-equipment-repair.phase` ns docstring 'Actuation'
      section) -- the SAME escalation shape `cloud-itonami-isic-3319`
      uses, extended here even though this class carries a genuinely
      higher intrinsic electrical hazard than 3319's residual scope,
      because checks 3-6 above (forbidden-action-class including
      `:re-energization-sign-off?`, equipment-verification, legal-basis,
      unresolved-concern) already gate the real hazard surface
      independently of phase -- the schedule op itself is still only ever
      a proposed WINDOW, never a live-work authorization.
  ...
  (:require [electrical-equipment-repair.store :as store]))
...
(def high-stakes
  ...
  surface stays aligned with `cloud-itonami-isic-3319`'s. `:order-
  supplies` is deliberately NOT a permanent member either -- its
...
(defn- forbidden-action-class-violations
  ...
  / re-energization-sign-off marker. Permanent, structural, un-overridable
  by any human approval -- see ns docstring check 3."
  [proposal]
  (let [v (:value proposal)]
    (when (and (map? v)
               (or (true? (:repair-equipment-control? v))
                   ...
                   (true? (:re-energization-sign-off? v))))
      [{:rule :forbidden-action-class
        :detail "電気設備/診断ツールの直接操作コマンド、または返品・稼働再開（return-to-service）・再通電（re-energization）の確定を伴う提案は恒久的に禁止（免許を持つ修理技術者の専権事項）"}])))
...
(defn- legal-basis-missing-violations
  ...
  source -- never invent a jurisdiction's pre-repair de-energization/
  re-energization requirements. See ns docstring check 5."
  [{:keys [op]} proposal]
  (when (= op :schedule-repair-operation)
    (let [value (:value proposal)]
      (when (or (empty? (:cites proposal))
        ...
        [{:rule :no-legal-basis
          :detail "公式legal-basisの引用が無い提案は電気設備修理作業スケジュール提案として扱えない"}]))))
    ...
    (let [a (store/equipment st subject)]
      (when (true? (:safety-concern-unresolved? a))
        [{:rule :unresolved-safety-concern
          :detail (str subject " は未解決の安全性懸念（絶縁不良/アーク・フラッシュ危険/修理未完了）がある状態での電気設備修理作業スケジュール提案")}]))))
