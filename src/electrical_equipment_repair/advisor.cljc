(ns electrical-equipment-repair.advisor
  "Repair Advisor client -- the *contained intelligence node* for the
  electrical-equipment-repair OPERATIONS COORDINATION actor.

  It normalizes repair-record logging (diagnostic-finding/repair-work-
  performed/parts-used data for motors, generators, transformers and
  switchgear), drafts a diagnostic/repair/testing SCHEDULE PROPOSAL
  (never a repair-equipment/diagnostic-tool control command or a
  return-to-service/re-energization sign-off), drafts a safety-concern
  FLAG (insulation failure, arc-flash risk, incomplete repair), and
  drafts a replacement-parts SUPPLY-ORDER PROPOSAL. CRITICAL: it is a
  smart-but-untrusted advisor, and it is SCOPED -- it holds NO
  repair-equipment/diagnostic-tool control authority and NO
  return-to-service/re-energization sign-off authority (that is the
  licensed repair technician's exclusively). It returns a *proposal*
  (with a rationale + the fields it cited), NEVER a committed record, a
  real mail/phone send, a real procurement order, or a real
  return-to-service/re-energization authorization. Every output carries
  `:effect :propose` and is censored downstream by `electrical-equipment-
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     ...
     :effect     :propose       ; ALWAYS :propose -- see `electrical-equipment-repair.governor`
...
(defn- schedule-repair-operation
  ...
  command or a return-to-service/re-energization sign-off; see
  `electrical-equipment-repair.governor` ns docstring). `:stake
  ...
  (let [a (store/equipment db subject)
    ...
    (if (nil? sb)
      {:summary    (str iso3 " の公式legal-basisが見つかりません -- スケジュール提案不可")
       :rationale  "electrical-equipment-repair.facts に未登録の法域。要件を推測で作らない。"
       :cites      []
       :effect     :propose
       :value      {:equipment-id subject :jurisdiction iso3 :window window :notes notes :spec-basis nil}
       :stake      :schedule-repair-operation
       :confidence 0.2}
      {:summary    (str subject " 向け電気設備修理作業スケジュール提案 (" (:owner-authority sb) ")"
                        (when a (str " (equipment=" (:name a) ")")))
       :rationale  (str "停電/通電前安全basis: " (:repair-safety-basis sb)
...
(defn- flag-safety-concern
  "Draft a SAFETY-CONCERN FLAG -- surfacing an electrical-hazard
  (insulation failure, arc-flash risk) or incomplete-repair concern for
  human review. `:stake :flag-safety-concern` -- ALWAYS escalates to a
  human, unconditionally, regardless of confidence (see README
  `Actuation` + `electrical-equipment-repair.governor` ns docstring
  `high-stakes`)."
  [db {:keys [subject concern-type concern-description]}]
  (let [a (store/equipment db subject)
        sb (facts/spec-basis (:jurisdiction a))]
    {:summary    (str subject ": 安全性懸念を検出（" (name (or concern-type :insulation-failure)) "）"
                      (when a (str " (equipment=" (:name a) ")")))
     :rationale  (if sb
     ...
     :value      {:equipment-id subject
                  :concern-type (or concern-type :insulation-failure)
                  :concern-description concern-description
                  :subject-line (str "[至急] " (:name a) " 安全性懸念のお知らせ")
                  :body (str (:name a) "について安全性懸念（" (name (or concern-type :insulation-failure))
...
(defn- order-supplies
  ...
  supplies` -- escalates when `:cost-usd` exceeds `electrical-equipment-
  repair.governor/supply-order-cost-threshold-usd`, or when confidence is
  low (see `electrical-equipment-repair.governor`)."
...
(def ^:private system-prompt
  (str "あなたは電気設備修理工房の運行調整（オペレーションズ・コーディネーション）"
       ...
       "重要: あなたは電気設備/診断ツールを直接操作するコマンドや、返品・稼働再開"
       "（return-to-service）・再通電（re-energization）の確定を伴う提案を絶対に"
       "作成してはいけません（免許を持つ修理技術者の専権事項）。登録されていない"
       "法域の要件を絶対に創作してはいけません。legal-basisが無い場合は:citesを"
       "空にしconfidenceを上げないこと。"))
