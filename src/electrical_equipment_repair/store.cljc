(ns electrical-equipment-repair.store
  "SSoT for the electrical-equipment-repair-coordination actor, behind a
  ...
  (test/electrical_equipment_repair/store_contract_test.clj), which is
  the whole point: the actor, the Repair Governor and the audit ledger
  ...
  number of times for the same equipment/work-order (a motor, generator,
  transformer or switchgear unit gets logged repeatedly over its repair
  lifecycle, may be rescheduled, may accumulate multiple safety-concern
  flags, may place multiple supply orders) BECAUSE this actor never
  actually operates repair equipment/diagnostic tools or signs off on
  return-to-service/re-energization -- it only ever proposes, logs and
  schedules (`:effect :propose` unconditionally, see `electrical-
  equipment-repair.governor` ns docstring). The equipment/work-order's
  own `:equipment-verified?` / `:safety-concern-unresolved?` ground-truth
  fields are what the Repair Governor independently re-checks before a
  `:schedule-repair-operation` / `:flag-safety-concern` / `:order-
  supplies` proposal may ever commit (see `electrical-equipment-repair.
  governor`).
  ...
  immutable log."
  (:require [electrical-equipment-repair.registry :as registry]
...
(defprotocol Store
  ...
  (all-equipment [s])
  (ledger [s])
  (repair-record-log-history [s] "the append-only repair-record-log history (electrical-equipment-repair.registry drafts)")
...
(defn demo-data
  ...
  []
  {:equipment
   {"equipment-1" {:id "equipment-1" :name "Kōji Electric Motor Works -- 3-Phase Induction Motor Rewind (work order #5521)"
                   :jurisdiction "JPN" :equipment-verified? true
                   :safety-concern-unresolved? false
                   :safety-contacts [{:name "Kōji (repair technician)" :email "koji@example.com" :phone "+819000000021"}
                                     {:name "Aiko (shop safety officer)" :email "aiko@example.com" :phone "+819000000022"}]
                   :status :ready-to-schedule}
    "equipment-2" {:id "equipment-2" :name "Atlantis Power Systems -- Distribution Transformer Repair"
                   :jurisdiction "ATL" :equipment-verified? true
                   :safety-concern-unresolved? false
                   :safety-contacts []
                   :status :intake}
    "equipment-3" {:id "equipment-3" :name "山田電機修理工房 発電機オーバーホール"
                   :jurisdiction "JPN" :equipment-verified? false
                   :safety-concern-unresolved? false
                   :safety-contacts [{:name "Yamada (repair technician)" :email "yamada@example.com" :phone "+819000000023"}]
                   :status :unverified}
    "equipment-4" {:id "equipment-4" :name "ことぶき配電盤サービス 低圧switchgear修理"
                   :jurisdiction "JPN" :equipment-verified? true
                   :safety-concern-unresolved? true
                   :safety-contacts [{:name "Watanabe (repair technician)" :email "watanabe@example.com" :phone "+819000000024"}
                                     {:name "Yamamoto (shop safety officer)" :email "yamamoto@example.com" :phone "+819000000025"}]
                   :status :concern-open}
    "equipment-5" {:id "equipment-5" :name "Riverside Municipal Utility -- Standby Generator Control Panel Repair"
                   :jurisdiction "USA" :equipment-verified? true
                   :safety-concern-unresolved? false
                   :safety-contacts [{:name "Casey (repair technician)" :email "casey@example.com" :phone "+15550000021"}]
                   :status :ready-to-schedule}
    "equipment-6" {:id "equipment-6" :name "Alt-Elektromotorenwerkstatt -- Servomotor-Reparatur"
                   :jurisdiction "DEU" :equipment-verified? true
                   :safety-concern-unresolved? false
                   :safety-contacts [{:name "Müller (repair technician)" :email "mueller@example.com" :phone "+4915000000021"}]
                   :status :ready-to-schedule}}})
